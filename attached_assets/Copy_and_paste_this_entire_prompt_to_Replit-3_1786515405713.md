...
Use read-only schema inspection before writing anything. The active API source uses `knowledge_topics` for Library topics and a direct `knowledge_sources.topic_id` relationship for evidence. Confirm this in production before writes. A historical `knowledge_topic_sources` junction table may exist, but do not use it unless the active production graph route is confirmed to read it.

If production differs, adapt only the actual Library evidence table names and relationship model. Do not infer or alter unrelated tables.
...
3. use the active direct source-to-topic model: upsert `knowledge_sources` by `(topic_id, normalized canonical URL)` when `knowledge_sources.topic_id` is the graph route's relationship; use a junction mapping only if production schema inspection and the active graph route both prove it is required;
4. mark valid source records `active` only when the source directly supports that topic; do not invent a mapping scope column if production does not have one;
5. store source name, canonical URL, authority tier, status, supported confidence/provenance fields, `last_verified` or `retrieved_at`, and a concise relevance note;
...
3. Machine-readable topic coverage manifest (CSV or JSON) containing:
   ...
   - authority tier;
   - source-link model and active status;
   - last verified date;
