...
**Author:** Manus AI  
**Status:** Implementation specification — no production data or aliases created  
...
A member must be able to share a public post with `#beyonceNYCconcert` and a video. A permitted search for **Beyoncé concerts**, **Beyoncé NYC**, **Bey**, or **Queen B** should find the post when the post is public or visible to that searcher. The result must be labeled as a member-shared video or post; it cannot be presented as an official Beyoncé event, verified artist news, or Library fact merely because the tag matches.

A member must also be able to propose community location language. For example, a proposed relationship such as `Uptown` → `Mount Airy, Philadelphia` is valid only **within the stated Philadelphia geographic context** after the proposal has reached the evidence/review threshold. It must not cause Kinfolk to resolve `Uptown` in Atlanta, New York, Chicago, or another city as Philadelphia. A community-contributed alias can improve interpretation but cannot override a user’s explicit city, address, map selection, or exact place name.
...
| --- | --- | --- | --- |
| **Post tag / compound hashtag** | `#beyonceNYCconcert` | Helps members find related posts and media. | The post stores it immediately after normal community policy checks. It becomes searchable only inside the reader’s visibility permissions. |
| **Cultural entity alias** | `Bey` → Beyoncé Knowles-Carter; `Queen B` → Beyoncé Knowles-Carter | Resolves a person, artist, work, organization, or culture topic in Kinfolk search. | Requires source-backed registry entry or curator-approved community threshold. |
| **Community geographic alias** | `Uptown` → a proposed community region within Philadelphia | Interprets a place term inside a defined geographic scope. | Requires a place scope, provenance, confidence, and curator/community validation. |
...
For a search such as `Beyoncé concerts NYC`, evaluate in this order:
...
A public post tagged `#beyonceNYCconcert` can score for this request only if its resolved entity, place, and intent support the query. It cannot appear first for `Beyoncé Atlanta concert` merely because `Beyoncé` has high semantic similarity.
...
| --- | --- | --- |
| `Bey` | Beyoncé Knowles-Carter | Return an entity-interpreted result with a small explanation if ambiguity is material. |
| `Queen B` | Beyoncé Knowles-Carter **only if source-backed/approved** | Expand only after the cultural alias record is active. |
| `Beyonce NYC` | Beyoncé + New York City | Require the geographic match; do not return every Beyoncé post. |
| `Natalie` | No default person | Ask “Which Natalie do you mean?” or show disambiguation choices. |
...
- `“Uptown in Philly”` can resolve only against active Philadelphia-scoped candidates.
- `“Uptown Atlanta”` can resolve only against active Atlanta-scoped candidates.
- `“Uptown”` with no city and no trustworthy explicit context must ask: **“Do you mean Uptown in which city?”** It must not assume Philadelphia because a member previously saved Mount Airy or because a large number of community posts use that term.
...
The founder’s phrase “search everything across the web” should be implemented as **broad, governed public-web research**, not unrestricted scraping or automatic belief.
...
| Search diverse cultural, regional, diaspora, Spanish-language, and English-language sources when query/context warrants it. | Infer a member’s ethnicity, language identity, political views, religion, health status, or nationality from name, appearance, location, or browsing history. |
...
| ALIAS-02 | Search `Beyoncé concerts NYC`. | The authorized post is retrievable through canonical entity + NYC + concert mapping and labeled `Member shared`. |
...
| ALIAS-04 | Search `Beyoncé Atlanta concert`. | NYC-only post does not rank as an Atlanta result. |
...
Replit must submit one Railway deployment proof package containing: migration and index proof; anonymized seed fixture for the Beyoncé tag/entity and the two city-scoped Uptown aliases; a three-account visibility transcript; query logs showing City hard filtering; active alias provenance; threshold/fingerprint proof without personal identifiers; browser screenshots/video for ALIAS-01 through ALIAS-09; and all automated-test results.
...
[3] [FEMA — Wireless Emergency Alerts](https://www.fema.gov/emergency-managers/practitioners/integrated-public-alert-warning-system/public/wireless-emergency-alerts)
