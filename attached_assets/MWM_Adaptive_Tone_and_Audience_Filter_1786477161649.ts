...
export type SourceConfidence = 'authoritative_current' | 'verified' | 'credible' | 'community' | 'unknown';
...
// -----------------------------------------------------------------------------
// 2. Helpers: explicit profile only; conservative defaults
// -----------------------------------------------------------------------------
...
export async function loadAdaptiveDeliveryProfile(
  ...
  if (error) throw new Error(`ADAPTIVE_PROFILE_LOAD_FAILED:${error.code}`);

  // Safe defaults. No permission is inferred from missing preferences.
  return {
...
function hasSensitiveTopic(tags: string[]): boolean {
  const blocked = new Set([
    'medical', 'mental_health', 'fertility', 'hiv', 'sexual_health', 'divorce',
    'relationship', 'immigration', 'financial_distress', 'trauma', 'domestic_violence',
  ]);
  return tags.some((tag) => blocked.has(tag.toLowerCase()));
}

// -----------------------------------------------------------------------------
// 3. Answer delivery: “quick” vs “deep” is explicitly chosen, never assumed.
// -----------------------------------------------------------------------------
...
export function buildDeliveryInstructions(
  ...
  const depth = {
    ...
    standard: { maxSections: 4, maxBullets: 5, includeCitationDetails: plan.citationMode === 'required' },
...
// -----------------------------------------------------------------------------
// 4. Audience eligibility for notifications/digests. This does NOT decide what
// someone can ask. It only limits what the platform may proactively push.
// -----------------------------------------------------------------------------
...
export function evaluateAudienceEligibility(
  ...
  const civicOrTraumatic = content.containsCivicOrPoliticalContent || content.containsTraumaticContent;
  ...
  if (content.kind === 'safety_alert' && content.isOfficialAlert && content.isCurrent) {
    ...
    return { eligible: true, reason: 'eligible', allowedPresentation: minor ? 'age_appropriate' : 'essential_alert_only' };
  }
  ...
  if ((content.kind === 'notification' || content.kind === 'city_digest') && !profile.allowNonSensitiveRecommendations) {
    return { eligible: false, reason: 'not_explicitly_opted_in' };
  }
  ...
  return { eligible: true, reason: 'eligible', allowedPresentation: minor ? 'age_appropriate' : 'direct' };
}

// -----------------------------------------------------------------------------
// 5. Express middleware: attach the delivery plan after router/auth middleware.
// -----------------------------------------------------------------------------

export function createAdaptiveDeliveryMiddleware(deps: { supabase: SupabaseClient }) {
  return async function adaptiveDeliveryMiddleware(
    ...
    try {
      ...
      const profile = await loadAdaptiveDeliveryProfile(deps.supabase, req.user.id);
...
/*
import { createUniversalSearchRouterMiddleware } from './universal-search-router';
import { createAdaptiveDeliveryMiddleware } from './adaptive-tone-and-audience-filter';

const universalRouter = createUniversalSearchRouterMiddleware({ openai, supabase, audit });
const adaptiveDelivery = createAdaptiveDeliveryMiddleware({ supabase });

// Existing auth middleware must run first. Keep the existing Kinfolk chat handler.
app.post(
  '/api/kinfolk/chat',
  requireAuthenticatedUser,
  universalRouter,
  adaptiveDelivery,
  existingKinfolkChatHandler,
);

// In existingKinfolkChatHandler:
// const systemPrompt = buildAdaptiveAnswerSystemPrompt(
//   req.kinfolk.deliveryInstructions,
//   req.kinfolk.route,
// );
*/
