...
export const userPreferencesTable = pgTable("user_preferences", {
  ...
  recommendationLifeStage: varchar("recommendation_life_stage", { length: 20 })
    .notNull()
    .default("unspecified"),
  ...
  communicationStyle: varchar("communication_style", { length: 20 }).default(
    "friendly",
  ),
  personalityMode: varchar("personality_mode", { length: 30 }).default(
    "neighborhood_guide",
  ),
  ...
  kinfolkVoice: varchar("kinfolk_voice", { length: 20 })
    .notNull()
    .default("onyx"),
  ...
  // safety, resource, or business results.
  communities: jsonb("communities").$type<string[]>().default([]),
  cultures: jsonb("cultures").$type<string[]>().default([]),
  preferredLanguages: jsonb("preferred_languages")
    .$type<string[]>()
    .default([]),
  personalizationContextCompletedAt: timestamp(
  ...
  searchHistory: jsonb("search_history")
    .$type<
      Array<{ query: string; type: string; categories: string[]; ts: number }>
    >()
    .default([]),
