...
export const usersTable = pgTable("users", {
  ...
  trialReminder3DaySentAt: timestamp("trial_reminder_3day_sent_at", {
    withTimezone: true,
  }),
  trialReminder1DaySentAt: timestamp("trial_reminder_1day_sent_at", {
    withTimezone: true,
  }),
  ...
  kinfolkQueriesThisMonth: integer("kinfolk_queries_this_month")
    .notNull()
    .default(0),
  ...
  policyViolationsCount: integer("policy_violations_count")
    .notNull()
    .default(0),
  ...
  // Set only when an administrator provides a temporary password. It is cleared
  // by the member's first successful password change; no password is exposed.
  ...
  displayNameFormat: varchar("display_name_format", {
    enum: ["full", "first_only", "first_last_initial"],
  }).default("full"),
  ...
  isCommunityOrganizer: boolean("is_community_organizer")
    .notNull()
    .default(false),
  profileSetupComplete: boolean("profile_setup_complete")
    .notNull()
    .default(false),
  ...
  quietHoursFrom: varchar("quiet_hours_from", { length: 10 })
    .notNull()
    .default("10:00 PM"),
  quietHoursUntil: varchar("quiet_hours_until", { length: 10 })
    .notNull()
    .default("8:00 AM"),
  ...
  testingEntitlementEndsAt: timestamp("testing_entitlement_ends_at", {
    withTimezone: true,
  }),
...
export const authEventsTable = pgTable(
  ...
  (t) => [
    index("IDX_auth_events_user_id").on(t.userId),
    index("IDX_auth_events_created_at").on(t.createdAt),
  ],
);
