export default function BizSlide12Signature() {
    ...
    <div className="relative w-screen h-screen overflow-hidden flex flex-col" style={{ background: "#1C0E06" }}>
      ...
      <div className="absolute inset-0">
        <svg viewBox="0 0 900 620" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
          ...
          {/* Spoke lines — all flowing toward center */}
          {satellites.map((sat, i) => {
            ...
            return (
              <line key={i}
                ...
                stroke="rgba(202,146,43,0.2)" strokeWidth="1" strokeDasharray="3,5"
              />
            );
          })}
            ...
            return (
              <g key={i}>
                ...
                <text x={lp.x} y={lp.y} textAnchor={anchor} fill="#D9C4A3" fontSize="10" fontFamily="sans-serif" fontWeight="600">
                  {lines.map((line, li) => (
                    <tspan key={li} x={lp.x} dy={li === 0 ? "-0.4em" : "1.3em"}>{line}</tspan>
                  ))}
                </text>
              </g>
            );
          ...
          {/* Map/pin icon */}
          <path
            d={`M ${cx} ${cy - 22} c-9,0 -14,7 -14,14 c0,10 14,26 14,26 s14,-16 14,-26 c0,-7 -5,-14 -14,-14 z`}
            fill="none" stroke="#CA922B" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"
          />
          ...
          {/* Inward flow arrows on spokes — small chevrons toward center */}
          {satellites.map((sat, i) => {
            const midR = r * 0.55;
