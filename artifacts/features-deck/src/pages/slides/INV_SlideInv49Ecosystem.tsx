...
export default function SlideInv49Ecosystem() {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden" style={{ background: "#3D2417" }}>
      ...
      <div className="absolute left-[6vw] right-[6vw] top-[3vw] text-center">
        ...
        <div className="font-display" style={{ fontSize: "3.2vw", fontWeight: 800, color: "#FAF6EF", lineHeight: 1.1 }}>Businesses don't grow alone.</div>
      </div>
      ...
      <div className="absolute" style={{ left: "10vw", right: "10vw", top: "11vw", bottom: "5.5vw" }}>
        <svg viewBox="0 0 600 520" width="100%" height="100%">
          {satellites.map((sat, i) => {
            ...
            return <line key={i} x1={inner.x} y1={inner.y} x2={outer.x} y2={outer.y} stroke="rgba(202,146,43,0.22)" strokeWidth="1" strokeDasharray="3,4" />;
          })}
          ...
          {satellites.map((sat, i) => {
            ...
            return (
              <g key={i}>
                ...
                <svg x={p.x - 7} y={p.y - 7} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#CA922B" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  {iconPaths[sat.icon]}
                </svg>
                <text x={lp.x} y={lp.y} textAnchor={anchor} fill="#D9C4A3" fontSize="11" fontFamily="sans-serif" fontWeight="600">
                  {labelLines.map((line, li) => <tspan key={li} x={lp.x} dy={li === 0 ? "-0.3em" : "1.3em"}>{line}</tspan>)}
                </text>
              </g>
            );
          })}
