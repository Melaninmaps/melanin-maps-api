...
function requireFounder(req: Request, res: Response): { id: string } | null {
  ...
  if (!isAdmin(req)) {
    res
      .status(403)
      .json({ error: "Founder/admin access required", code: "ADMIN_REQUIRED" });
    return null;
  }
  return { id: user.id };
}
...
for (const address of ["192.0.0.9", "192.0.0.10"])
  GLOBAL_SPECIAL_IPV4.addAddress(address, "ipv4");
for (const [network, prefix] of [
  ["2001:1::1", 128],
  ["2001:1::2", 128],
  ["2001:1::3", 128],
  ...
  GLOBAL_SPECIAL_IPV6.addSubnet(network, prefix, "ipv6");
...
export function isDirectoryEvidencePublicIp(address: string): boolean {
  ...
  if (family === 4)
    return (
      GLOBAL_SPECIAL_IPV4.check(address, "ipv4") ||
      !NON_GLOBAL_IPV4.check(address, "ipv4")
    );
  if (family === 6)
    return (
      GLOBAL_SPECIAL_IPV6.check(address, "ipv6") ||
      !NON_GLOBAL_IPV6.check(address, "ipv6")
    );
  return false;
}

export function areDirectoryEvidenceAddressesPublic(
  addresses: string[],
): boolean {
  return (
    addresses.length > 0 &&
    addresses.every((address) => isDirectoryEvidencePublicIp(address))
  );
}
    ...
    const request = (secure ? httpsRequest : httpRequest)(
      {
        ...
        headers: {
          Host: url.host,
          "User-Agent":
            "MappingWithMelanin-StagingDirectoryReview/1.0 (https://mappingwithmelanin.com)",
...
export async function validateDirectoryEvidenceUrl(
  ...
  const resolveAddresses =
    networking.resolveAddresses ?? resolvePublicAddresses;
  ...
  for (let redirect = 0; redirect <= 3; redirect += 1) {
    ...
    if (!addresses || !areDirectoryEvidenceAddressesPublic(addresses))
      return null;
    let response = await request(current, addresses[0]!, "HEAD");
    if (response?.status === 405)
      response = await request(current, addresses[0]!, "GET");
...
function signUrlValidation(
  payload: UrlValidationPayload,
  secret: string,
): string {
  if (secret.length < 32)
    throw new Error(
      "Directory review signing secret must contain at least 32 characters.",
    );
...
export function signDirectoryUrlValidation(
  ...
  validation: Pick<
    ...
    "url" | "finalHost" | "status" | "result" | "checkedAt"
  >,
  secret: string,
): string {
...
function verifyUrlValidation(
  ...
  try {
    ...
    if (
      ...
      new URL(canonicalSource).hostname.toLowerCase().replace(/^www\./, "") !==
        payload.finalHost ||
...
const NON_FACTUAL_PUBLIC_TAG =
  /\b(?:black|african[ -]?american|minority|women?|female|male|lgbtq\+?|queer|trans(?:gender)?|veteran|disabled|disability|children?|kids?|teens?|youth|under[ -]?13|adult(?:s)?[ -]?only|21\+|safe(?:ty)?|verified|trusted|approved|best|excellent|affordable|budget|luxury|free|pricing|price)\b/i;

type SourceBackedListingEnrichment = Readonly<{
...
const OFFICIAL_ENRICHMENT_EVIDENCE = "official_destination_page_reviewed";
const NON_FACTUAL_ENRICHMENT =
  /\b(?:black|african[ -]?american|minority|women?|female|male|lgbtq\+?|queer|trans(?:gender)?|veteran|disabled|disability|verified|trusted|approved|best|excellent|affordable|budget|luxury|free|pricing|price)\b/i;

function sourceBackedListingEnrichment(
  candidate: Pick<DirectoryImportCandidate, "raw_record">,
): SourceBackedListingEnrichment {
  const raw = candidate.raw_record;
  if (raw.enrichmentEvidence !== OFFICIAL_ENRICHMENT_EVIDENCE) {
    return {
      description: null,
      hours: null,
      searchTags: [],
        ...
        source
          ...
          .filter(
            (value) =>
              value.length >= 2 &&
              value.length <= 120 &&
              !NON_FACTUAL_ENRICHMENT.test(value),
          ),
      ),
    ]
  ...
  const description = asTrimmedString(raw.sourceBackedDescription, 700);
  const hours = asTrimmedString(raw.publishedHours, 255);
  return {
    description,
    hours,
    searchTags: values("searchTags", 24),
    specialtySlugs: values("specialtySlugs", 12, /^[a-z0-9]+(?:-[a-z0-9]+)*$/),
    audiencesServed: values("audiencesServed", 12),
    accessibilityFeatures: values("accessibilityFeatures", 12),
    amenityTags: values("amenityTags", 12, /^[a-z0-9]+(?:-[a-z0-9]+)*$/),
  };
}

export function sourceBackedPublicSearchTags(
  candidate: Pick<DirectoryImportCandidate, "raw_record">,
): string[] {
  const usesOfficialEnrichment =
    candidate.raw_record.enrichmentEvidence === OFFICIAL_ENRICHMENT_EVIDENCE;
  if (
    !usesOfficialEnrichment &&
    candidate.raw_record.publicSearchTagEvidence !==
      "workbook_category_services_and_reviewed_offerings_only"
  )
    return [];
  ...
  for (const rawTag of rawTags) {
    ...
    if (
      ...
      tag.length > 90 ||
      (usesOfficialEnrichment
        ? NON_FACTUAL_ENRICHMENT
        : NON_FACTUAL_PUBLIC_TAG
      ).test(tag)
    )
      continue;
...
function freshAcceptedLink(
  ...
  if (
    !ACCEPTED_LINK_RESULTS.has(
      typeof evidence.result === "string" ? evidence.result : "",
    )
  )
    return null;
  ...
  const finalHost =
    asTrimmedString(evidence.finalHost, 300)
      ?.toLowerCase()
      .replace(/^www\./, "") ?? null;
  if (
    !finalUrl ||
    !finalHost ||
    new URL(finalUrl).hostname.toLowerCase().replace(/^www\./, "") !== finalHost
  )
    return null;
  return finalUrl;
}
...
function parseRegulatedEvidence(
  value: unknown,
  candidateId: string,
  secret: string,
  ...
  if (
    ...
    !sourceUrl ||
    !validation ||
    !isOfficialAuthorityUrl(validation.url)
  ) {
    return null;
  }
  return {
...
function locationCandidateFingerprint(
  candidate: Pick<
    DirectoryImportCandidate,
...
export function signDirectoryLocationSuggestion(
  ...
  if (secret.length < 32)
    throw new Error(
      "Directory location signing secret must contain at least 32 characters.",
    );
...
function parseLocationEvidence(
  ...
  try {
    const payload = JSON.parse(
      Buffer.from(encoded, "base64url").toString("utf8"),
    ) as LocationSuggestionPayload;
    const expiresAt = new Date(payload.expiresAt).getTime();
    if (
      payload.candidateId !== candidate.id ||
      payload.candidateFingerprint !==
        locationCandidateFingerprint(candidate) ||
      !Number.isFinite(expiresAt) ||
      expiresAt < Date.now() ||
      !freshCheckedAt(payload.checkedAt) ||
...
function parseCoordinatesFromGeocoder(
  ...
  const resolvedStateCode = Object.entries(address).find(([key]) =>
  ...
  const returnedStreet = foldLocation(
  ...
  const cityMatches = Boolean(
    expectedCity &&
    resolvedCity &&
    (resolvedCity === expectedCity ||
      resolvedCity.includes(expectedCity) ||
      expectedCity.includes(resolvedCity)),
  );
  const stateMatches = Boolean(
    expectedState &&
    (stateCode === expectedState || resolvedState === expectedState),
  );
  const houseMatches = Boolean(
    expectedHouse && returnedHouse && expectedHouse === returnedHouse,
  );
  const streetMatches =
    streetTokens.length > 0 &&
    streetTokens.some((token) => returnedStreet.includes(token));
  const osmId =
...
export async function geocodeDirectoryCandidate(
  ...
  const query = [candidate.address, candidate.city, candidate.state, "USA"]
    .filter(Boolean)
    .join(", ");
...
function buildReviewEvidence(
  ...
  const regulated = parseRegulatedEvidence(
    body.regulatedEvidence,
    candidate.id,
    signingSecret,
  );
  const resource = parseResourceEvidence(
  ...
  return {
    ...
    resourceSourceTier: VALID_RESOURCE_TIERS.has(
      body.resourceSourceTier as ResourceSourceTier,
    )
...
export function evaluatePublicationHolds(
  ...
  if (candidate.target_kind === "internal_only")
    holds.push("Internal-only candidates cannot be published.");
  if (candidate.target_kind === "manual_review")
    holds.push(
  ...
  if (
    reviewGates(candidate).includes("duplicate_within_batch") &&
    body.action !== "link_existing"
  ) {
    holds.push(
      "An in-batch duplicate must link to the canonical record after reconciliation.",
    );
  }
  if (
    body.action !== "link_existing" &&
    hasUnresolvedLink(candidate) &&
    body.linkEvidenceConfirmed !== true
  ) {
  ...
  if (
    body.action !== "link_existing" &&
    (candidate.regulated_profession ||
  ...
  if (candidate.target_kind === "community_resource") {
    if (
      !VALID_RESOURCE_CATEGORIES.has(body.resourceCategory as ResourceCategory)
    ) {
    ...
    if (
      requestedTier === "official" &&
      resourceEvidence &&
      !isGovernmentUrl(resourceEvidence.sourceUrl)
    ) {
      holds.push(
        "Official resource tier requires a current government source URL.",
      );
    }
  }
  return holds;
...
async function loadCandidateForUpdate(
  id: string,
  client: Queryable,
): Promise<DirectoryImportCandidate | null> {
...
async function appendDecisionEvent(
  ...
  await client.query(
    ...
    [
      ...
      input.newStatus,
      input.reviewNote,
      JSON.stringify(input.reviewEvidence),
      input.idempotencyKey,
      input.payloadHash,
      input.publishedRecordType ?? null,
      input.publishedRecordId ?? null,
    ],
  );
}
...
async function publishBusiness(
  ...
  const canonicalCategory = getBusinessExperiencePolicy(
    candidate.category,
  ...
  const requestedExistingId =
    asTrimmedString(body.existingRecordId, 100) ??
    candidate.matched_business_id;
  ...
  if (body.action === "link_existing") {
    ...
    if (
      !requestedExistingId ||
      !existing ||
      existing.id !== requestedExistingId
    ) {
      throw new RouteError(
        409,
        "CANONICAL_MATCH_NOT_FOUND",
        "The selected canonical business was not found.",
      );
    }
  ...
  if (!coordinates) {
    throw new RouteError(
      ...
      "A reviewed non-zero location is required before this business can be published.",
    );
  }
  ...
  const tiktok = validatedSocialUrl(
    "tiktok",
  ...
  const socialProfiles = [
    ...
    url ? [{ platform, url, handle: null, suppliedByUser: false }] : [],
  );
  const ownershipDesignations =
    body.ownershipEvidenceConfirmed === true &&
    body.omitOwnershipDesignations !== true
      ? (candidate.ownership_designations ?? [])
      : [];
  const regulatedEvidence = parseRegulatedEvidence(
    body.regulatedEvidence,
    candidate.id,
    signingSecret,
  );
  const enrichment = sourceBackedListingEnrichment(candidate);
  ...
  const sourceEvidence = [
    {
      ...
      ownershipEvidence:
        body.ownershipEvidenceConfirmed === true
          ? candidate.ownership_evidence
          : null,
      regulatedEvidence,
      publicSearchTags,
      excerpt:
        "Founder-curated external source reviewed for an unclaimed directory listing; this is not Mapping With Melanin ownership or business verification.",
    },
  ];
  const description =
  ...
  await client.query(
    ...
    [
      ...
      websiteHost(website),
      enrichment.hours,
      candidate.price_basis ? candidate.price_range : null,
      JSON.stringify(publicSearchTags),
      instagram,
      tiktok,
      facebook,
      JSON.stringify(socialProfiles),
      JSON.stringify(sourceEvidence),
      JSON.stringify(ownershipDesignations),
      hasBlackOwnedDesignation(ownershipDesignations),
      reviewerId,
      normalizeText(candidate.name),
      canonicalDedupeKey,
      candidate.id,
  ...
  if (enrichment.specialtySlugs.length > 0) {
    await client.query(
      `INSERT INTO business_specialties (business_id, specialty_slug)
       SELECT $1::uuid, UNNEST($2::text[])
       ON CONFLICT DO NOTHING`,
      [businessId, enrichment.specialtySlugs],
    );
  }
  if (
    enrichment.audiencesServed.length > 0 ||
    enrichment.accessibilityFeatures.length > 0 ||
    enrichment.amenityTags.length > 0
  ) {
    await client.query(
      `INSERT INTO business_identity (business_id, audiences_served, accessibility_features, amenity_tags)
       VALUES ($1, $2::jsonb, $3::jsonb, $4::jsonb)
       ON CONFLICT (business_id) DO UPDATE
       SET audiences_served = EXCLUDED.audiences_served,
           accessibility_features = EXCLUDED.accessibility_features,
           amenity_tags = EXCLUDED.amenity_tags,
           updated_at = NOW()`,
      [
        businessId,
        JSON.stringify(enrichment.audiencesServed),
        JSON.stringify(enrichment.accessibilityFeatures),
        JSON.stringify(enrichment.amenityTags),
      ],
    );
  }
...
async function publishResource(
  ...
  const tier = VALID_RESOURCE_TIERS.has(requestedTier)
    ? requestedTier
    : "community_shared";
  if (
    (tier === "official" || tier === "verified_org") &&
    !resourceEvidence.sourceUrl
  ) {
    throw new RouteError(
      400,
      "RESOURCE_SOURCE_REQUIRED",
      "Official and verified organization tiers require current source evidence.",
    );
  }
  ...
  if (body.action === "link_existing") {
    ...
    if (!requestedExistingId || !linked || linked.id !== requestedExistingId) {
      throw new RouteError(
        409,
        "RESOURCE_MATCH_NOT_FOUND",
        "The selected resource does not match this candidate's reviewed canonical identity.",
      );
    }
    await client.query(
      `INSERT INTO directory_import_publications
  ...
  const memberUrl =
    safeHttpUrl(body.memberFacingUrl) ?? resourceEvidence.sourceUrl;
  const keywords = [
    ...new Set(
      [
        candidate.name,
  ...
  const organization =
    resourceEvidence.organization ?? candidate.source_name ?? candidate.name;
  ...
  await client.query(
    ...
    [
      ...
      candidate.state,
      keywords,
      new Date(resourceEvidence.checkedAt),
      canonicalKey,
      normalizeText(candidate.name),
      candidate.category,
      candidate.subcategory,
  ...
  await client.query(
    ...
    [
      ...
      candidate.category,
      candidate.subcategory,
      JSON.stringify(candidate.raw_record),
      JSON.stringify(candidate.link_validation),
      reviewerId,
...
export function registerDirectoryImportRoutes(
  ...
  const locationSigningSecret =
    dependencies.locationSigningSecret ??
    process.env.DIRECTORY_REVIEW_SIGNING_SECRET ??
    "";
  const validateEvidenceUrl =
    dependencies.validateEvidenceUrl ?? validateDirectoryEvidenceUrl;

  app.get(
    "/api/founder/directory-import-batches",
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        req.log?.error(
          { err: error },
          "Failed to list directory import batches",
        );
        res
          .status(500)
          .json({ error: "Failed to load directory import batches." });
      }
    },
  );

  app.get(
    "/api/founder/directory-import-batches/:batchId",
    async (req: Request, res: Response) => {
      if (!requireFounder(req, res)) return;
      try {
        const result = await transactionPool.query(
        ...
        res
          .status(500)
          .json({ error: "Failed to load directory import batch." });
      }
    },
  );

  app.get(
    "/api/founder/directory-import-candidates",
    async (req: Request, res: Response) => {
      if (!requireFounder(req, res)) return;
      ...
      const targetKind = asTrimmedString(
        req.query.targetKind ?? req.query.target_kind,
        50,
      );
      ...
      for (const [queryName, column, max] of [
        ["batchId", "c.batch_id", 80],
        ["city", "c.city", 100],
        ["state", "c.state", 50],
        ["category", "c.category", 120],
      ] as const) {
      ...
      if (q) {
        ...
        conditions.push(
          `(c.name ILIKE ${placeholder} OR c.subcategory ILIKE ${placeholder} OR c.address ILIKE ${placeholder} OR c.source_name ILIKE ${placeholder})`,
        );
      }
      const regulated = asTrimmedString(req.query.regulated, 10);
      if (regulated === "true" || regulated === "false")
        add("c.regulated_profession = ?", regulated === "true");
      ...
      try {
        ...
        req.log?.error(
          { err: error },
          "Failed to list directory import candidates",
        );
        res
          .status(500)
          .json({ error: "Failed to load directory import candidates." });
      }
    },
  );

  app.get(
    "/api/founder/directory-import-candidates/:id",
    async (req: Request, res: Response) => {
      if (!requireFounder(req, res)) return;
      try {
        ...
        res.json({
          candidate,
          events: events.rows,
          publicationHolds: reviewGates(candidate),
        });
      } catch (error: unknown) {
        req.log?.error(
          { err: error },
          "Failed to load directory import candidate",
        );
        res
          .status(500)
          .json({ error: "Failed to load directory import candidate." });
      }
    },
  );

  app.post(
    "/api/founder/directory-import-candidates/:id/validate-evidence-url",
    async (req: Request, res: Response) => {
      if (!requireFounder(req, res)) return;
      try {
        const candidate = await loadCandidate(
        ...
        const purpose =
          req.body?.purpose === "regulated" || req.body?.purpose === "resource"
        ...
        if (!purpose || !submittedUrl) {
          res
            .status(400)
            .json({
              error: "A valid public URL and evidence purpose are required.",
            });
          return;
        }
        if (
          ...
          res
            .status(409)
            .json({
              error: "Regulated evidence is not applicable to this candidate.",
            });
          return;
        }
        if (
          ...
          res
            .status(409)
            .json({
              error: "Resource evidence is not applicable to this candidate.",
            });
          return;
        }
        if (locationSigningSecret.length < 32) {
          res
            .status(503)
            .json({ error: "Directory evidence review is not configured." });
          return;
        }
        ...
        if (!validation) {
          res
            .status(409)
            .json({
              error: "The source URL did not pass the live public-link check.",
              code: "EVIDENCE_URL_NOT_VALIDATED",
            });
          return;
        }
        if (
          ...
          res
            ...
            .json({
              error:
                "Regulated evidence must resolve to an approved government or configured authority host.",
              code: "AUTHORITY_HOST_REQUIRED",
            });
          return;
        }
        res.json({
          validation,
          validationToken: signDirectoryUrlValidation(
            candidate.id,
            purpose,
            validation,
            locationSigningSecret,
          ),
          message:
            "Live URL validation passed. Complete the remaining review fields before publication.",
        });
      } catch (error: unknown) {
        req.log?.error(
          { err: error },
          "Failed to validate directory evidence URL",
        );
  ...
  app.post(
    "/api/founder/directory-import-candidates/:id/location-suggestion",
    async (req: Request, res: Response) => {
      if (!requireFounder(req, res)) return;
      try {
        const candidate = await loadCandidate(
        ...
        if (!suggestion) {
          res.status(409).json({
            error:
              "No address result passed the city, state, street, and house-number checks. Enter reviewed location evidence manually.",
        ...
        if (locationSigningSecret.length < 32) {
          res
            .status(503)
            .json({
              error: "Directory location review is not configured.",
              code: "LOCATION_SIGNING_UNAVAILABLE",
            });
          return;
        }
        res.json({
          suggestion,
          suggestionToken: signDirectoryLocationSuggestion(
            candidate,
            suggestion,
            locationSigningSecret,
          ),
          message:
            "Review the resolved address and source, then explicitly confirm it before publication.",
        });
      } catch (error: unknown) {
        req.log?.error(
          { err: error },
          "Failed to prepare directory candidate location suggestion",
        );
        res
          .status(500)
          .json({ error: "Failed to prepare a location suggestion." });
      }
    },
  );

  app.post(
    "/api/founder/directory-import-candidates/:id/decision",
    async (req: Request, res: Response) => {
      ...
      if (
        !action ||
        !["publish", "link_existing", "needs_research", "decline"].includes(
          action,
        )
      ) {
        res
          ...
          .json({
            error:
              "action must be publish, link_existing, needs_research, or decline",
      ...
      try {
        const replay = await replayedDecision(
          transactionPool,
          idempotencyKey,
          payloadHash,
        );
        if (replay) {
          if (replay.candidate_id !== candidateId) {
            throw new RouteError(
              409,
              "IDEMPOTENCY_CANDIDATE_MISMATCH",
              "This idempotency key belongs to a different candidate.",
            );
          }
          ...
          res.json({
            ...
            replayed: true,
            candidate,
            recordType: replay.published_record_type,
            recordId: replay.published_record_id,
          });
          return;
        }
        ...
        if (action === "publish" && isBusinessTarget(preflight)) {
          ...
          if (!coordinates) {
            throw new RouteError(
              409,
              "LOCATION_EVIDENCE_REQUIRED",
              "A reviewer-confirmed, unexpired server-validated location suggestion is required before this business can be published.",
            );
          }
        }
        ...
        if (candidate.review_revision !== expectedRevision) {
          throw new RouteError(
            ...
            "This candidate changed after it was loaded. Refresh before deciding.",
            { currentRevision: candidate.review_revision },
          );
        }
        if (
        ...
        if (action === "needs_research" || action === "decline") {
          ...
          const holds = evaluatePublicationHolds(
            candidate,
            body,
            locationSigningSecret,
          );
          if (holds.length > 0) {
            throw new RouteError(
              409,
              "PUBLICATION_HELD",
              "This candidate still has required review holds.",
              { holds },
            );
          }
          if (isBusinessTarget(candidate)) {
            ...
            published = await publishBusiness(
              candidate,
              founder.id,
              body,
              coordinates,
            ...
            published = await publishResource(
              candidate,
              founder.id,
              body,
              coordinates,
            ...
            throw new RouteError(
              ...
              "This candidate must remain in manual review.",
            );
          }
          newStatus = "published";
        }
        ...
        const updated = await client.query<DirectoryImportCandidate>(
          ...
          [
            ...
            published?.recordType ?? null,
            published?.recordId ?? null,
            expectedRevision,
          ],
        );
        if (!updated.rows[0])
        ...
        if (
          ...
          res
            ...
            .json({
              error:
                "This candidate or canonical identity was already published.",
        ...
        req.log?.error(
          { err: error },
          "Failed to process directory import decision",
        );
        res
          .status(500)
          .json({ error: "Failed to process directory import decision." });
      } finally {
        client?.release();
      }
    },
