...
export async function findExactRecords(
  ...
  const state = query.location.stateCode
    ? query.location.stateCode.toUpperCase()
    : null;
  const specialty = query.filters.specialty ?? null;
  const category = query.filters.category ?? null;
  const ownershipValueGroups = normalizeOwnershipDesignationFilterIds(
    query.filters.ownership ?? [],
  ).map((id) => ownershipDesignationStorageValues(id).values);
  const searchableQuery = query as SearchableLocationFirstQuery;
  const searchText =
    searchableQuery.filters.searchText ?? searchableQuery.searchText ?? null;
  ...
  if (recordTypes.includes("business")) {
    ...
    if (specialty) {
      ...
      specialtyClause = `AND bs.specialty_slug = $${params.length}`;
    }
    let categoryClause = "";
    if (category) {
  ...
  if (recordTypes.includes("cultural_site")) {
    ...
    results.push(
      ...rows.map((r) => ({
        ...
        longitude: r.lng ? parseFloat(r.lng) : null,
        distanceMiles: null,
        detailUrl: `/tour-cultural-sites/${r.id}`,
        isVerified: true,
        contextTags: [],
      })),
    );
  ...
  if (recordTypes.includes("event")) {
    ...
    if (dateRange === "today")
      dateClause =
        "AND re.active_until >= CURRENT_DATE AND re.active_until < CURRENT_DATE + INTERVAL '1 day'";
    else if (dateRange === "weekend")
      dateClause =
        "AND re.active_until >= date_trunc('week', CURRENT_DATE) + INTERVAL '5 days' AND re.active_until < date_trunc('week', CURRENT_DATE) + INTERVAL '8 days'";
    else if (dateRange === "month")
      dateClause =
        "AND re.active_until >= CURRENT_DATE AND re.active_until < CURRENT_DATE + INTERVAL '30 days'";
    else
      dateClause =
  ...
  if (recordTypes.includes("community_place")) {
    ...
    results.push(
      ...rows.map((r) => ({
        id: r.id,
        recordType: "community_place" as const,
        name: r.name,
        category: r.mission?.slice(0, 50) ?? null,
        specialty: null,
        city: r.city,
        stateCode: r.state_code,
        neighborhood: r.neighborhood,
...
export async function findNearestAvailableLocation(
  ...
  if (queryLat && queryLng) {
    ...
    if (distRows[0]) {
      return {
        city: distRows[0].city_name,
        stateCode: distRows[0].state_code,
        distanceMiles: parseFloat(distRows[0].dist),
      };
    }
  }
