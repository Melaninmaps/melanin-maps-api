...
export async function sendReferralNudge(
  ...
  const weeklyLine = newSignupsThisWeek > 0
    ? `<strong>${newSignupsThisWeek} ${newSignupsThisWeek === 1 ? "person" : "people"}</strong> joined the waitlist this week — share your link to move ahead of them!`
    : `Keep sharing your link to climb the waitlist faster.`;

  await resend.emails.send({
    from: FROM,
    to,
    subject: `You're #${position} on the Mapping With Melanin™ waitlist 🗺️`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#FAF6EF;padding:40px 32px;border-radius:16px">
        ...
        <p style="color:#3A1F0E;font-size:16px;line-height:1.6;margin:0 0 16px">
          You're still <strong style="color:#CA922B">#${position}</strong> on the Mapping With Melanin™ waitlist.
          ${weeklyLine}
        </p>
        ...
        <div style="background:#2B1507;border-radius:12px;padding:24px;margin-bottom:24px">
          <p style="color:#F5EBD8;font-size:14px;font-weight:700;margin:0 0 12px;text-align:center;letter-spacing:1px">── Your Referral Link ──</p>
          <p style="color:#F5EBD8;font-size:15px;line-height:1.6;margin:0 0 16px;text-align:center">
            Every friend who joins using your link moves you up. Share it widely!
          </p>
          <a href="${referralLink}" style="display:block;background:#CA922B;color:#fff;font-weight:700;font-size:15px;padding:14px 24px;border-radius:50px;text-decoration:none;text-align:center;margin-bottom:16px">
            Share My Referral Link →
          </a>
          <p style="color:#F5EBD8;font-size:13px;margin:0 0 6px;opacity:0.8;text-align:center">Or copy your link:</p>
          <p style="color:#CA922B;font-size:13px;word-break:break-all;text-align:center;margin:0">${referralLink}</p>
          <p style="color:#F5EBD8;font-size:13px;margin:16px 0 0;text-align:center;opacity:0.7">Your referral code: <strong style="letter-spacing:3px">${referralCode}</strong></p>
        </div>

        <p style="color:#3A1F0E;font-size:14px;line-height:1.6;margin:0 0 16px;opacity:0.7">
          We're building something special for our community. Early access slots are limited — referrals are the fastest way to the front of the line.
        </p>
        ...
        <p style="color:#3A1F0E;font-size:12px;opacity:0.4;margin:24px 0 0;text-align:center">
          You're receiving this because you joined the Mapping With Melanin™ waitlist.<br/>
          <a href="https://www.melaninmaps.com" style="color:#CA922B">melaninmaps.com</a>
        </p>
      </div>
    `,
  });
