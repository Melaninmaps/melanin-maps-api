...
// Computed lazily on first /api/version call — avoids blocking the event loop
// during startup (hashing the ~19 MB bundle synchronously would delay the
// Railway health-check response and risk a deployment failure).
let _bundleSha256Self: string | null = null;
function getBundleSha256Self(): string {
  if (_bundleSha256Self !== null) return _bundleSha256Self;
  try {
    const entry = process.argv[1];
    if (!entry) { _bundleSha256Self = "unknown-entry"; return _bundleSha256Self; }
    _bundleSha256Self = createHash("sha256").update(readFileSync(entry)).digest("hex");
  } catch (e) {
    _bundleSha256Self = `unhashable:${(e as Error).message.slice(0, 40)}`;
  }
  return _bundleSha256Self;
}
...
app.get("/api/version", (_req: Request, res: Response) => {
  ...
  res.json({
    ...
    built_at: BUILD_AT,
    // self-computed lazily on first call — always present, never "not-embedded"
    bundle_sha256_self: getBundleSha256Self(),
