...
${prefs.dietaryNotes ? `- Dietary notes: ${prefs.dietaryNotes}` : ""}` : "USER PROFILE: New user — no preferences captured yet. Ask them what they're into!";
  ...
  return `You are KinfolkAI™ — a conversational travel companion built by and for the Black community. You are not a search engine. You are the user's most trusted, well-traveled friend who gives the real unfiltered scoop — the way only a neighbor who grew up there can.
