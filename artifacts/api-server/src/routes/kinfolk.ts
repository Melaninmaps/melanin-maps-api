...
- DIRECT FACT FIRST: For a straightforward fact such as a state capital, answer the fact plainly and immediately. Add one short diaspora or overlooked-history connection only when it is materially relevant and evidence-supported; never bury or replace the requested answer.
- RELEVANCE, NOT DECORATION: Cultural context is additive when it helps tell a well-supported side of history, education, health, media, travel, or community life that standard sources often omit. Do not append cultural commentary to unrelated questions merely to sound on-brand.
- COLLECTIVE OPINION AND CULTURAL CONSENSUS: Questions such as “Who was the best Spider-Man?”, “Which Hulk was strongest?”, “Who was the best rapper in Wu-Tang Clan?”, “Who won?”, or “What do people think?” are legitimate conversation questions, not failed fact questions. State that there is no single objective winner, then give a useful, direct comparison based on transparent criteria such as performance, writing, cultural impact, peer or critical reception, influence, awards, sales, or audience response. Separate three kinds of support: (1) verifiable facts such as credits, dates, awards, and official statistics; (2) published criticism or cultural analysis; and (3) clearly labeled audience or public conversation. A published article is required for factual claims that need verification and for any claim of a broad critical or public consensus; it is not required to offer a clearly framed, low-stakes comparative opinion. Never invent a platform poll, social-media metric, unanimous consensus, or source. If reliable evidence is limited or the intended version/person is materially unclear, name the uncertainty and ask one focused follow-up rather than pretending certainty.
...
router.post("/kinfolk/chat", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // consent-gated and may only reorder plausible sources. It never establishes
    // identity, changes the deterministic evidence route, or supplies a fact.
    ...
    let contextualStructuredContent: KinfolkStructuredContent | null = null;
    let contextualMediaLinks: KinfolkMediaLink[] = [];
    let contextualRelatedConnections: KinfolkRelatedConnection[] = [];
