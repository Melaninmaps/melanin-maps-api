...
import {
  ...
  businessPromotionsTable,
  businessSearchInquiriesTable,
  userPreferencesTable,
  businessClickEventsTable,
  businessCaptionsTable,
  contentReportsTable,
  referenceLinkClicksTable,
...
function categoryFilterStorageValues(raw: string): string[] {
  return (
    CATEGORY_FILTER_ALIASES[raw.trim().toLocaleLowerCase("en-US")] ?? [
      raw.trim(),
    ]
  );
}
function toPublicBusinessRecord<T extends Record<string, unknown>>(
  business: T,
) {
  const {
    ...
    await withDbRetry(
      async () => {
        ...
        const offset = Math.max(
          0,
          parseInt((offsetParam as string) ?? "0", 10) || 0,
        );
        const pageLimit = Math.min(
          200,
          Math.max(1, parseInt((limitParam as string) ?? "200", 10) || 200),
        );
        ...
        const geoRadiusMi = radiusParam
          ? parseFloat(radiusParam as string)
          : 25;
        const hasGeoFilter =
          geoLat !== null &&
          geoLng !== null &&
          ...
          conditions.push(
            or(
              ...categoryValues.map((value) =>
                eq(businessesTable.category, value),
              ),
            )!,
          );
        }

        // Every requested designation is an explicit AND requirement. This enables an
        // intentional search such as Black + Woman + D9 + Veteran without inferring
        // any label from business names, categories, photos, or geography.
        const requestedDesignationValues = [
        ...
        if (search && typeof search === "string") {
          ...
          if (tokens.length <= 1) {
            ...
            const allInName = tokens.map((t) =>
              ilike(businessesTable.name, `%${t}%`),
            );
            const allInDesc = tokens.map((t) =>
              ilike(businessesTable.description, `%${t}%`),
            );
            const allInTags = tokens.map(
              (t) =>
                sql<boolean>`COALESCE(${businessesTable.tags}, '[]'::jsonb)::text ILIKE ${`%${t}%`}`,
            );
            const allInRecord = tokens.map(
              (t) => sql<boolean>`CONCAT_WS(' ',
        ...
        if (city && typeof city === "string") {
          ...
          conditions.push(
            sql`LOWER(BTRIM(COALESCE(${businessesTable.city}, ''))) = LOWER(BTRIM(${canonicalCity}))`,
          );
        }

        if (state && typeof state === "string") {
          conditions.push(
            sql`UPPER(BTRIM(COALESCE(${businessesTable.state}, ''))) = UPPER(BTRIM(${state}))`,
          );
        }
        ...
        if (subcategory && typeof subcategory === "string") {
          conditions.push(
            ilike(businessesTable.subcategory, `%${subcategory}%`),
          );
        }
        ...
        const activePromos = await db
          .select({
            businessId: businessPromotionsTable.businessId,
            type: businessPromotionsTable.type,
          })
        ...
        const promotedIdToType = new Map(
          activePromos.map((p) => [p.businessId, p.type]),
        );
        ...
        const matchesPref = (b: { ownershipDesignations: string[] }) =>
          ...
          callerPrefs.some((preference) =>
            (b.ownershipDesignations ?? [])
              .map(ownershipDesignationFilterId)
              .includes(ownershipDesignationFilterId(preference)),
          );
        ...
        const annotated = businesses
          ...
          .sort((a, b) => {
            ...
            if (!a.culturalMatch && b.culturalMatch) return 1;
            // 5. Founding members
            if (b.foundingBusiness !== a.foundingBusiness)
              return b.foundingBusiness ? 1 : -1;
            // 6. Popularity: confidence score + rating × log(reviewCount+1) boost
            const aScore =
              (a.confidenceScore ?? 0) +
              Number((a as any).rating ?? 0) *
                Math.log(Number((a as any).reviewCount ?? 0) + 1) *
                0.1;
            const bScore =
              (b.confidenceScore ?? 0) +
              Number((b as any).rating ?? 0) *
                Math.log(Number((b as any).reviewCount ?? 0) + 1) *
                0.1;
        ...
        const withCaptions = annotated.map((b) => ({
          ...b,
          topCaptions: captionMap.get(b.id) ?? [],
        }));
          ...
          try {
            ...
            if (tokens.length > 1) {
              ...
              const score = sql<number>`GREATEST(${sql.join(
                tokens.map(
                  (token) =>
                    sql`similarity(LOWER(${businessesTable.name}), LOWER(${token}))`,
                ),
                sql`, `,
              )})`;
              fuzzyRows = await db
            ...
            if (fuzzyRows.length === 0) {
              ...
              fuzzyRows = await db
                ...
                .from(businessesTable)
                .where(
                  and(...fuzzyConditions, sql<boolean>`${phraseScore} > 0.22`),
                )
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        count,
        spots,
        remaining: Math.max(0, spots - count),
        isFull: count >= spots,
      });
    } catch (err) {
...
router.patch("/businesses/mine/profile", async (req: any, res: Response) => {
  ...
  if (category) {
    ...
    updates.subcategory = subcategory?.trim() || category;
  }
  ...
  if (primarySocialPlatform !== undefined)
    updates.primarySocialPlatform = primarySocialPlatform || null;
  if (businessTagline !== undefined)
    updates.businessTagline = businessTagline?.trim() || null;
    ...
    try {
      const [business] = await db
        .select({
          ...
          pendingPhotos: businessesTable.pendingPhotos,
        })
        .from(businessesTable)
        .where(approvedOwnerPredicate(String(userId)));
      if (!business) {
      ...
      const safeExt = ["jpg", "jpeg", "png", "webp", "heic", "heif"].includes(
        ext,
      )
        ? ext
        : "jpg";
      ...
      res.status(201).json({
        ...
        pending: true,
        message:
          "Photo submitted for review. It will appear on your profile once approved.",
...
router.post(
  "/businesses/:id/community-photos",
  photoUpload.single("photo"),
  async (req: any, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
    ...
    try {
      ...
      if (currentPhotos.length + currentPending.length >= 20) {
        res
          .status(400)
          .json({ error: "This business has reached its photo limit" });
        return;
      }
      ...
      res.status(201).json({
        ...
        pending: true,
        message:
          "Photo submitted — thank you! It will appear once our team approves it.",
      });
    } catch (err) {
...
router.get(
  "/admin/businesses/pending-photos",
  async (req: any, res: Response) => {
    if (!isAdmin(req)) {
...
router.post(
  "/admin/businesses/:id/photos/approve",
  async (req: any, res: Response) => {
    if (!isAdmin(req)) {
      res.status(403).json({ error: "Admin access required" });
      return;
    ...
    try {
      ...
      res.json({
        ...
        pendingPhotos: updated.pendingPhotos,
...
router.post(
  "/admin/businesses/:id/photos/reject",
  async (req: any, res: Response) => {
    if (!isAdmin(req)) {
      res.status(403).json({ error: "Admin access required" });
      return;
    ...
    try {
      ...
      await db
        .update(businessesTable)
        .set({ pendingPhotos: newPending, updatedAt: new Date() })
        .where(eq(businessesTable.id, businessId));
      const bucketId = process.env.DEFAULT_OBJECT_STORAGE_BUCKET_ID;
  ...
  async (req: any, res: Response) => {
    ...
    try {
      ...
      if (!currentPhotos.includes(url)) {
        res.status(400).json({ error: "Photo not found in your gallery" });
        return;
      }
  ...
  async (req: any, res: Response) => {
    ...
    try {
      ...
      if (currentVideos.length >= 5) {
        res.status(400).json({ error: "Maximum of 5 videos allowed" });
        return;
      }
      ...
      const safeExt = ["mp4", "mov", "avi", "webm", "mkv", "m4v"].includes(ext)
        ? ext
        : "mp4";
...
router.delete("/businesses/mine/videos", async (req: any, res: Response) => {
  ...
  try {
    ...
    const updatedVideos = ((business.videos as string[]) ?? []).filter(
      (v) => v !== url,
    );
  ...
  async (req: any, res: Response) => {
    ...
    try {
      const [business] = await db
        .select({
          id: businessesTable.id,
          introVideoUrl: businessesTable.introVideoUrl,
        })
        .from(businessesTable)
        .where(approvedOwnerPredicate(String(userId)));
      if (!business) {
      ...
      if (
        bucketId &&
        business.introVideoUrl?.includes(`storage.googleapis.com/${bucketId}/`)
      ) {
        const objectKey = business.introVideoUrl.split(
          `storage.googleapis.com/${bucketId}/`,
        )[1];
        if (objectKey)
          objectStorageClient
            .bucket(bucketId)
...
router.post(
  "/businesses/mine/featured-video/upload",
  ...
  async (req: any, res: Response) => {
    ...
    try {
      const [business] = await db
        .select({
          id: businessesTable.id,
          featuredVideoTitle: businessesTable.featuredVideoTitle,
          featuredVideoPurpose: businessesTable.featuredVideoPurpose,
        })
        .from(businessesTable)
        .where(approvedOwnerPredicate(String(userId)));
      if (!business) {
...
router.delete(
  "/businesses/mine/featured-video/hosted",
  async (req: any, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ error: "Unauthorized" });
    ...
    try {
      const [business] = await db
        .select({
          id: businessesTable.id,
          featuredVideoUrl: businessesTable.featuredVideoUrl,
        })
        .from(businessesTable)
        .where(approvedOwnerPredicate(String(userId)));
      if (!business) {
      ...
      if (
        bucketId &&
        business.featuredVideoUrl?.includes(
          `storage.googleapis.com/${bucketId}/`,
        )
      ) {
        const objectKey = business.featuredVideoUrl.split(
          `storage.googleapis.com/${bucketId}/`,
        )[1];
        if (objectKey)
          objectStorageClient
...
router.patch(
  "/businesses/mine/weekly-schedule",
  async (req: any, res: Response) => {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ error: "Unauthorized" });
    ...
    try {
      ...
      if (showAvailability !== undefined)
        updates.showAvailability = showAvailability;
      ...
      res.json({
        ...
        showAvailability: updated.showAvailability,
      });
    } catch (err) {
...
router.get("/businesses/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const containsDemoMarker = [business.name, business.description].some(
      (value) =>
        typeof value === "string" && value.toLowerCase().includes("[demo]"),
    );
    ...
    void (async () => {
      if (userId) {
        const [settings] = await db
          .select({
            profileViewTrackingEnabled:
              userSettingsTable.profileViewTrackingEnabled,
          })
...
router.post(
  "/businesses/community-reference",
  async (req: any, res: Response) => {
    try {
      if (!req.user?.id) {
      ...
      req.log.info(
        { businessId: business.id, submittedBy: req.user.id },
        "Community reference listing queued for review",
      );
...
router.post("/businesses/suggest-place", async (req: any, res: Response) => {
  try {
    ...
    if (dupeRows.rows.length > 0) {
      const existing = dupeRows.rows[0] as {
        id: string;
        name: string;
        city: string;
      };
    ...
    res
      .status(500)
      .json({ error: "Failed to create place. Please try again." });
  }
});
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existing =
        await communitySubmissionRepository.findPublishedDuplicate(input);
      if (existing) {
        res.status(409).json({
      ...
      const result = await communitySubmissionRepository.create(
        input,
        req.user!.id,
      );
      ...
      if (
        /required|must be|invalid|unsupported|accepts at most|too long/i.test(
          message,
        )
      ) {
...
router.post("/businesses/:id/view", async (req: Request, res: Response) => {
  try {
    ...
    await db
      .insert(businessProfileViewsTable)
      .values({ businessId: id, userId });
    res.json({ ok: true });
  } catch (err) {
...
router.post("/businesses/:id/click", async (req: Request, res: Response) => {
  try {
    ...
    await db
      .insert(businessClickEventsTable)
      .values({ businessId: id, userId, clickType });
    res.json({ ok: true });
  } catch (err) {
...
router.post(
  "/businesses/:id/reference-link-click",
  async (req: Request, res: Response) => {
    try {
      const id = String(req.params.id);
      const userId = req.user?.id ?? null;
...
router.get(
  "/businesses/:id/reference-analytics",
  async (req: Request, res: Response) => {
    try {
      const id = String(req.params.id);
      ...
      const dailyTrend = await db
        .select({
          day: sql<string>`DATE(${referenceLinkClicksTable.clickedAt})`.as(
            "day",
          ),
          total: count(),
        })
        ...
        .where(
          and(
            eq(referenceLinkClicksTable.businessId, id),
            gt(
              referenceLinkClicksTable.clickedAt,
...
router.patch("/businesses/:id/address", async (req: Request, res: Response) => {
  try {
    ...
    void sendAddressUpdateNotifications(
      id,
      existing.name,
      oldAddress,
      newAddress,
...
router.patch("/businesses/:id/badges", async (req: Request, res: Response) => {
  try {
    ...
    const id = String(req.params.id);
    const { currentLocationSince, businessFoundedDate, trustBadges } =
      req.body as {
    ...
    const [business] = await db
      ...
      .set({
        ...
        trustBadges: Array.isArray(trustBadges)
          ? trustBadges
...
router.post(
  "/businesses/:id/seller-agreement",
  async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        res.status(401).json({ error: "Authentication required" });
      ...
      const [business] = await db
        ...
        .returning({
          id: businessesTable.id,
          sellerAgreementAcceptedAt: businessesTable.sellerAgreementAcceptedAt,
        });
...
router.patch(
  "/admin/businesses/:id/founding-status",
  async (req: Request, res: Response) => {
    try {
      if (!isAdmin(req)) {
      ...
      if (typeof founding !== "boolean") {
        res.status(400).json({ error: "founding must be true or false" });
        return;
      }
        ...
        void (async () => {
          try {
            ...
            const [owner] = await db
              ...
              .from(usersTable)
              .where(eq(usersTable.id, fullBiz.submittedById))
              .limit(1);
            ...
            void sendFoundingWelcomeEmail(
              ...
              req.log.error(
                { emailErr },
                "Founding welcome email failed — non-fatal",
              ),
            );
            ...
            try {
              ...
              req.log.error(
                { dsErr },
                "DocuSign founding agreement async trigger failed — non-fatal",
              );
            }
          } catch (err) {
            req.log.error(
              { err },
              "Founding async post-approval tasks failed — non-fatal",
            );
          }
        })();
      }
...
router.get(
  "/businesses/:id/marketplace-tier",
  async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        res.status(401).json({ error: "Authentication required" });
      ...
      if (!trialStartedAt) {
        ...
        await db
          .update(businessesTable)
          .set({
            businessTrialStartedAt: trialStartedAt,
            updatedAt: new Date(),
          })
          .where(eq(businessesTable.id, id));
      }
      const trialEndsAt = new Date(
        trialStartedAt.getTime() + BUSINESS_TRIAL_MS,
      );
      ...
      if (biz.foundingBusiness && biz.foundingGrantedAt) {
        ...
        if (elapsed < FOUNDING_WINDOW_MS) {
          foundingActive = true;
          foundingExpiresAt = new Date(
            new Date(biz.foundingGrantedAt).getTime() + FOUNDING_WINDOW_MS,
          );
        }
        // Founding businesses get 6 months of premium features from grant date
        const FOUNDING_PREMIUM_MS = 180 * 24 * 60 * 60 * 1000;
        foundingPremiumUntil = new Date(
          new Date(biz.foundingGrantedAt).getTime() + FOUNDING_PREMIUM_MS,
        );
      }
...
router.patch(
  "/admin/businesses/:id/marketplace-tier",
  async (req: Request, res: Response) => {
    try {
      if (!isAdmin(req)) {
        res.status(403).json({ error: "Admin required" });
    ...
    res.json(
      rows.map((r) => ({
        ...
        submittedBy: r.submitter_first_name
          ? [r.submitter_first_name, r.submitter_last_name]
              .filter(Boolean)
              .join(" ")
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (existing.length) {
        res
          .status(409)
          .json({ error: "You have already flagged this business" });
        return;
      }
  ...
  async (req: Request, res: Response) => {
    ...
    const KNOWN: Array<{
      ...
      },
      {
        ...
        longitude: "-75.1480",
        description:
          "Philadelphia's beloved Black-owned bookstore serving the community since 1959, stocking an unmatched selection of African American literature, history, culture, and children's books — operated by Mr. Hakim Rasul, a community institution in his own right.",
        ...
        await pool.query(
         ...
         ON CONFLICT (id) DO NOTHING`,
          [
            ...
            biz.instagram ?? null,
            biz.blackOwned,
            JSON.stringify(biz.ownershipDesignations),
...
router.post(
  "/admin/businesses/:id/clear-dispute",
  async (req: Request, res: Response): Promise<void> => {
    if (!isAdmin(req)) {
      res.status(403).json({ error: "Admin required" });
      return;
...
router.post(
  "/admin/businesses/:id/photos/upload",
  ...
  async (req: any, res: Response): Promise<void> => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        await gcsFile.makePublic();
        uploadedUrls.push(
          `https://storage.googleapis.com/${bucketId}/${objectKey}`,
        );
      }
...
router.post(
  "/admin/businesses/:id/photos/delete",
  async (req: Request, res: Response): Promise<void> => {
    if (!isAdmin(req)) {
      res.status(403).json({ error: "Admin required" });
      return;
...
router.post(
  "/admin/businesses/:id/social-link",
  async (req: Request, res: Response): Promise<void> => {
    if (!isAdmin(req)) {
      res.status(403).json({ error: "Admin required" });
      return;
    ...
    try {
      ...
      if (adminNotes?.trim()) {
        try {
          ...
          /* Non-fatal — column may not exist on older Railway instances until migration runs */
        }
      }
      ...
      res
        .status(500)
        .json({ error: "Failed to create business. Please try again." });
    }
  },
);
    ...
    try {
      ...
      res.status(201).json({
        contribution: {
          ...
          status: "pending",
          message:
            "Your contribution has been submitted and will appear after review — usually within 24 hours.",
        },
      });
    } catch (err) {
      ...
      res
        .status(500)
        .json({ error: "Failed to submit contribution. Please try again." });
    }
  },
);
