...
export default function LoginScreen() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void (async () => {
      const stored = await SecureStore.getItemAsync("auth_session_token").catch(
        () => null,
      );
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      await refreshUser();
      router.replace(
        data.profileSetupComplete === false ? "/profile-setup" : "/(tabs)",
      );
    } catch (err: unknown) {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (result.mustChangePassword) {
          // The credential itself remains only in the authenticated session.
          // Do not pass the temporary password through route params or storage.
          router.replace("/set-initial-password" as any);
          return;
        }
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        ...
        >
          ...
          {connecting && (
            <View
              style={[
                styles.connectingBox,
                { backgroundColor: "#FFF7ED", borderColor: "#F59E0B40" },
              ]}
            >
              <Feather
                name="wifi-off"
              ...
              <View style={{ flex: 1, gap: 8 }}>
                <Text style={[styles.errorTxt, { color: "#B45309" }]}>
                  Signed in but could not reach the server. Check your
                  connection.
                </Text>
                <TouchableOpacity
                  activeOpacity={0.85}
          ...
          <View style={styles.dividerRow}>
            ...
            <Text style={[styles.dividerTxt, { color: c.mutedForeground }]}>
          ...
          {!emailMode ? (
            ...
            <View style={styles.emailForm}>
              ...
              <View>
                ...
                >
                  <Text style={[styles.forgotTxt, { color: c.primary }]}>
                    Forgot password?
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              >
                <Text style={[styles.switchTxt, { color: c.mutedForeground }]}>
          ...
          <View style={styles.signupRow}>
            <Text style={[styles.signupTxt, { color: c.mutedForeground }]}>
              New here?{" "}
            </Text>
            <TouchableOpacity
              activeOpacity={0.85}
            ...
            >
              <Text style={[styles.signupLink, { color: c.primary }]}>
