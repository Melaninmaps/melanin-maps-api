...
const failures = [];
if (config?.ios?.bundleIdentifier !== "com.melaninmaps.app") {
  failures.push("unexpected iOS bundle identifier");
}
if (config?.ios?.buildNumber !== "105") {
  failures.push(`expected build 105, found ${config?.ios?.buildNumber ?? "missing"}`);
}
if (backgroundModes.includes("audio")) {
  failures.push("generated UIBackgroundModes still contains audio");
}
