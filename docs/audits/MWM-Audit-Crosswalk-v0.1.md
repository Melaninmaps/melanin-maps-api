...
| Apple Sign-In | Route exists at `POST /auth/apple`. Identity token verified via Apple public keys. Nonce matching implemented. | Nonce handling does not meet iOS 26+ cryptographic requirement (SHA256 of rawNonce). Confirmed broken on iOS 26+. | WS-02 | ⚠️ |
