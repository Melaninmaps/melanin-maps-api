...
| Session revocation on logout | `GET /logout` and `POST /mobile-auth/logout` both call `deleteSession(sid)` — removes record from DB. Token is immediately invalid. `POST /auth/logout-all` added (Wave 1-A) — deletes all sessions for authenticated user via `pool.query` JSON path. | No "remember me" vs. short session distinction. Existing sessions before Wave 1-A deploy have no userId index but expire naturally. | WS-01 | ✅ |
| Rate limiting on auth endpoints | `authLimiter`: 30 requests / 15 min applied to `/api`. `generalLimiter`: 200 requests / 15 min. `failed_login_attempts` and `locked_until` added to `users` table (Wave 1-A). | Rate limit still IP-based for all other routes. Per-account lockout only on email login. | WS-01 | ✅ |
| Brute-force protection | `express-rate-limit` exists. **Wave 1-A:** `failed_login_attempts` tracked per account. 10 failures → 15-min lockout (HTTP 423). 20 failures → 60-min lockout. Counter reset on successful login. | No exponential backoff (progressive lockout instead). Phone OTP lockout handled by Twilio. | WS-01 | ✅ |
| Phone OTP registration and login | `POST /auth/phone/send-otp` and `POST /auth/phone/verify-otp` exist in `phone-auth.ts`. Twilio Verify integration. **Wave 1-A security fix:** `TEST_PHONE` bypass gated by `IS_PRODUCTION = process.env.NODE_ENV === "production"` — all 3 handler locations fixed. Regression test added (`src/__tests__/phone-auth-gate.test.ts`, 4 passing). | OTP delivery not verified end-to-end on production numbers in this sprint. | WS-02 | ✅ |
...
| Auth security audit log | `req.log.info` used for diagnostic events (AUTH_LOGIN_SUCCESS etc.). **Wave 1-A:** `auth_events` table added — records userId, event, ipAddress, userAgent, metadata, createdAt. Events: AUTH_LOGIN_SUCCESS, AUTH_LOGIN_FAILURE, AUTH_LOCKOUT_TRIGGERED, AUTH_LOCKED_ATTEMPT, LOGOUT_ALL. Non-fatal (pino remains primary; DB is durable second layer). | No record of Apple Sign-In events or password change events yet. | WS-01 | ✅ |
| Global token revocation (all devices) | Single-session logout works. **Wave 1-A:** `POST /auth/logout-all` added. `deleteAllSessionsForUser(userId)` uses `pool.query` with JSON path operator (`sess->'user'->>'id'`). Returns count of sessions revoked. Bearer token required. | Sessions created before this deploy have no indexed userId but will be deleted via JSON path scan. | WS-01 | ✅ |
...
| CRON_SECRET fail-closed | `verifyCronSecret` checks `x-cron-secret` header against `process.env.CRON_SECRET`. **Wave 3-A:** `if (!CRON_SECRET) return true` changed to `if (!CRON_SECRET) { res.status(401)... return false; }`. All cron endpoints now fail-closed when env var is unset. | Pre-deploy: must confirm CRON_SECRET is set in Railway env before next production deploy. | WS-14 | ✅ |
...
| Unsubscribe infrastructure | No unsubscribe links in email templates (`email.ts`). **Wave 3-B:** `sendEmail()` modified to append CAN-SPAM footer to ALL outbound HTML emails (one-point coverage). Footer includes HMAC-signed unsubscribe URL. `POST /api/auth/unsubscribe` route validates token and sets `marketing_opt_out=true` on user. `marketing_opt_out boolean default false` added to users table. | Per-category preference UI not yet built. Gating of cron mass emails on `marketing_opt_out` should be added to cron queries in next wave. | WS-11 | ✅ |
| Physical address in emails | "Melanin Maps LLC" and website included. **Wave 3-B:** CAN-SPAM footer appended to all emails via `sendEmail()`. Address from `COMPANY_MAILING_ADDRESS` env var, defaulting to "Melanin Maps LLC · Washington, DC". Must set `COMPANY_MAILING_ADDRESS` in Railway to full legal street address before launch. | `COMPANY_MAILING_ADDRESS` env var must be set to full physical address in production. | WS-11 | ✅ |
...
| Finding | Current Code State | Gap | WS | Status |
|---|---|---|---|---|
| CRON_SECRET fail-closed | `verifyCronSecret`: `if (!CRON_SECRET) return true` → **Wave 3-A fixed:** now returns 401 when env var is unset. Fail-closed on all 9 cron endpoint guards. | Pre-deploy: verify `CRON_SECRET` is set in Railway. | WS-14 | ✅ |
