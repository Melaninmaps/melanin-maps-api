...
const root = resolve(process.argv[2] ?? "/home/ubuntu/melanin-maps-inventory-staging/data/founder-imports/2026-09-18-midatlantic-combined-pass");
...
function canonicalTargetKind(value) {
  ...
  if (key === "online_business") return "online_business";
...
for (const file of files) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    try {
      ...
      const required = ["sourceRow", "targetKind", "name", "city", "country", "category", "sourceUrl"];
...
const consolidated = [...recordsByKey.values()]
  .sort((a, b) => a.record.city.localeCompare(b.record.city) || a.record.name.localeCompare(b.record.name))
  .map((item, index) => {
    let notes = {};
...
const summary = {
  ...
  files: {
    candidates: "mid-atlantic-consolidated-candidates.jsonl",
    exactDuplicates: "mid-atlantic-exact-duplicates-removed.json",
    destinationCollisions: "mid-atlantic-destination-collisions-review.json",
    invalidRowsHeld: "mid-atlantic-invalid-or-incomplete-rows-held.json",
  },
};
