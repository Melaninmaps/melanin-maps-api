...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-19-houston-family-profile-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "houston-family-profile-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "houston-family-profile-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    minneapolis_profile_review_package: {
...
const summary = {
  package: "2026-09-19-houston-family-profile-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "houston-family-profile-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "houston-family-profile-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "houston-family-profile-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "houston-family-profile-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "houston-family-profile-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
