...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-second-three-city-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "second-three-city-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "second-three-city-combined-destination-health.json");
...
const sourceHealthPayload = JSON.parse(await readFile(inputHealth, "utf8"));
...
for (const source of sourceRecords) {
  ...
  // An online-only business may have no supported physical locality. It must
  // remain addressless and mapless rather than receiving inferred geography.
  ...
  if (!isOnlineOnly && !hasStreetAddress(source.address)) reasons.push("missing_or_nonphysical_street_address");
  ...
  const record = {
    ...
    address: isOnlineOnly ? null : text(source.address),
    ...
    socialSourceUrl,
    servicesSearchTerms: text(source.servicesSearchTerms) || null,
    notes: "",
  };
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    second_three_city_review_package: {
...
const summary = {
  package: "2026-09-18-second-three-city-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "second-three-city-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
