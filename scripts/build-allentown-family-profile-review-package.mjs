...
const inputManifest = resolve(consolidatedDirectory, "allentown-family-profile-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "allentown-family-profile-combined-destination-health.json");
...
const summary = {
  package: "2026-09-19-allentown-family-profile-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "allentown-family-profile-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "allentown-family-profile-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "allentown-family-profile-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "allentown-family-profile-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "allentown-family-profile-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
