...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-international-three-city-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "international-three-city-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "international-three-city-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  const isMaplessNoncommercial = (kind === "community_resource" || kind === "cultural_place")
    && !hasStreetAddress(source.address);
  const reasons = [];
  ...
  // source-identified community resource or cultural place may likewise be
  // addressless without receiving an invented map location. All remain
  // mapless unless a later route has separate approved location handling.
  ...
  if (!isOnlineOnly && !isMaplessNoncommercial && !hasStreetAddress(source.address)) reasons.push("missing_or_nonphysical_street_address");
  ...
  const record = {
    ...
    address: isOnlineOnly || isMaplessNoncommercial ? null : text(source.address),
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    international_three_city_review_package: {
...
const summary = {
  package: "2026-09-18-international-three-city-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "international-three-city-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "international-three-city-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "international-three-city-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "international-three-city-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "international-three-city-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
