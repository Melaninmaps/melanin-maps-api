...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-next-three-city-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "next-three-city-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "next-three-city-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    next_three_city_review_package: {
...
const summary = {
  package: "2026-09-18-next-three-city-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "next-three-city-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
