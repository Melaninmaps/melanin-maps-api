...
const root = resolve(process.argv[2] ?? "/home/ubuntu/melanin-maps-inventory-staging/data/founder-imports/2026-09-18-international-three-city-review");
...
for (const file of files) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    try {
      ...
      const isMapless = targetKind === "online_business"
        || targetKind === "community_resource"
        || targetKind === "cultural_place";
      // Online-only services and source-identified community or cultural
      // places can be addressless and mapless. Commercial and regulated
      // candidates still require a specific locality and physical street
      // address before any later map-pin review.
...
const consolidated = [...recordsByKey.values()]
  ...
  .map((item, index) => {
    ...
    const record = {
      ...
      ...notes,
        international_three_city_consolidation: {
...
const summary = {
  ...
  files: {
    candidates: "international-three-city-consolidated-candidates.jsonl",
    exactDuplicates: "international-three-city-exact-duplicates-removed.json",
    destinationCollisions: "international-three-city-destination-collisions-review.json",
    invalidRowsHeld: "international-three-city-invalid-or-incomplete-rows-held.json",
  },
};
