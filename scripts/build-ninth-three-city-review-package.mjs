...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-ninth-three-city-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "ninth-three-city-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "ninth-three-city-combined-destination-health.json");
...
function customerDestination(value, source) {
  ...
  if (sourceLooksLikeDirectory && comparable && sourceComparable === comparable) return null;
  return url;
}
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    ninth_three_city_review_package: {
...
const summary = {
  package: "2026-09-18-ninth-three-city-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "ninth-three-city-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
const handoff = `# Combined Ninth Three City Source-Backed Directory Review Package\n\n**Status:** Review only. This package has not been staged to any database, published to the website, made visible in iOS or Android, added to the map, or supplied to live Kinfolk recommendations.\n\nThe package preserves ${sourceRecords.length} consolidated research candidates from the original ninth-three-city review package plus the second deep source pass. It contains ${accepted.length} structurally valid review candidates and holds ${held.length} records outside staging because they lack a usable customer-facing official destination after normalization. This is an evidence boundary, not a closure decision.\n\n## Review composition\n\n| Queue | Candidates |\n|---|---:|\n| Commercial business | ${targetCounts.business ?? 0} |\n| Online-only business | ${targetCounts.online_business ?? 0} |\n| Regulated review | ${targetCounts.regulated_review ?? 0} |\n| Community resource | ${targetCounts.community_resource ?? 0} |\n| Cultural place | ${targetCounts.cultural_place ?? 0} |\n| **Review candidates** | **${accepted.length}** |\n| Held for missing/invalid destination | ${held.length} |\n\n## Automated destination check\n\nThe review manifest contains ${healthPayload.uniqueDestinations} distinct current destinations. The automated check returned ${byOutcome.reachable ?? 0} reachable responses, ${byOutcome.review_required ?? 0} non-success responses requiring review, ${byOutcome.network_error ?? 0} network failures, ${byOutcome.timeout ?? 0} timeouts, and ${byOutcome.not_checked ?? 0} destinations that were not in the prior health report. A record enters the **needs-research** queue if any selected destination is not confirmed reachable. It is not removed or described as closed.\n\n## Approval boundary\n\nA founder/admin reviewer must reconcile duplicates against the live directory and approve individual commercial businesses. The protected importer then geocodes the published street address server-side before a commercial map pin exists. Regulated candidates require authority or licensing review. Community resources and cultural places follow their separate queues and must not become commercial map pins. Online-only businesses may be searchable after approval but must remain mapless.\n\n## Package integrity\n\nThe review manifest SHA-256 is ${manifestHash}. Do not edit the review manifest after this package is built. If a candidate changes, rebuild the package and rerun destination verification.\n\n## Source context\n\nThis package documents source-backed Memphis, Nashville, and Tulsa research. City-level evidence, source interpretation, first-party destination checks, original holds, and publisher-designation limits are maintained in the package source reports. Chamber or community-directory inclusion is not an ownership certification. Ownership or cultural designations appear only where the relevant source expressly supports them.\n\n## References\n\nSee the city-specific source reports included in this package.\n`;

await mkdir(outputDirectory, { recursive: true });
await Promise.all([
  writeFile(resolve(outputDirectory, "ninth-three-city-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "ninth-three-city-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "ninth-three-city-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "ninth-three-city-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
