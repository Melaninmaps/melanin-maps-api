...
function validateBuild106Policy() {
  ...
  for (const [name, contents] of [["root", rootEasIgnore], ["mobile", mobileEasIgnore]]) {
    for (const required of ["*.jks", "*.keystore", "*.pem", "*.p12", "*.p8", "*.key", "*.mobileprovision", "credentials.json", "google-service-account.json", "google-services.json", "GoogleService-Info.plist", ".replit", "replit.md", "replit.nix", "sedQ6qvzl", "upload_keystore.jks"]) {
