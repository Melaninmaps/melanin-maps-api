...
const [summaryText, candidateText, healthText] = await Promise.all([
...
if (summary.candidateManifest?.rowCount !== candidateRows || health.candidates !== candidateRows) {
  throw new Error("Candidate count does not reconcile across summary, manifest, and health ledger.");
}
if (!Array.isArray(health.results) || health.results.length !== health.uniqueDestinations || outcomeCount !== health.uniqueDestinations) {
  throw new Error("Destination health ledger does not reconcile its result count.");
}
if (health.results.some((result) => !allowedOutcomes.has(result.outcome))) {
  throw new Error("Destination health ledger contains an unsupported outcome.");
}

summary.destinationHealth = {
  path: "data/founder-imports/2026-09-20-multi-city-everyday-living-review/review-package/multi-city-everyday-living-destination-health.json",
...
console.log(JSON.stringify({
  candidateRows,
  healthSha256: summary.destinationHealth.sha256,
  uniqueDestinations: health.uniqueDestinations,
  counts: health.counts,
}, null, 2));
