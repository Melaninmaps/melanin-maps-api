#!/usr/bin/env bash
# Mapping With Melanin — surgical release executor for iOS 1.1.9 (115)
# and Android 1.1.7 (85). This script does NOT edit application code, delete
...
#   RELEASE_SHA=<full GitHub main SHA> bash scripts/replit-release-115-85.sh prepare
#   # review and commit only generated static assets, then use the resulting SHA:
#   RELEASE_SHA=<full final GitHub main SHA> bash scripts/replit-release-115-85.sh verify
#   RELEASE_SHA=<full final GitHub main SHA> bash scripts/replit-release-115-85.sh build
#
...
case "$MODE" in
  ...
  *)
    printf '%s\n' 'Usage: RELEASE_SHA=<full GitHub main SHA> bash scripts/replit-release-115-85.sh prepare|verify|build' >&2
    exit 64
    ;;
esac
...
require_exact_clean_source() {
  ...
  [ "$(jq -r '.expo.ios.buildNumber' artifacts/mobile/app.json)" = "115" ] || fail "iOS build number must remain 115"
  [ "$(jq -r '.expo.android.versionCode' artifacts/mobile/app.json)" = "85" ] || fail "Android versionCode must remain 85"
...
case "$MODE" in
  ...
  verify)
    ...
    printf 'VERIFY_PASS: exact_sha=%s ios=1.1.9(115) android=1.1.7(85)\n' "$RELEASE_SHA"
    ;;
  ...
  build)
    ...
    pnpm exec eas build \
      ...
      --no-wait \
      --non-interactive \
      --json > "$ROOT/release-eas-android-85.json"
