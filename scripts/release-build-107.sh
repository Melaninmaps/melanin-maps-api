...
usage() {
  printf '%s\n' \
    'Build 107 safe release entrypoint' \
    '' \
    'No action is taken without an explicit mode. No-argument and help execution are offline.' \
    '' \
    'Usage:' \
    '  MWM_BUILD107_SOURCE_SHA=<reviewed-40-char-sha> scripts/release-build-107.sh ios-testflight-staging' \
    '' \
    'Authorized action: iOS Build 107 using testflight-staging and frozen remote signing.' \
    'Blocked: production profiles, Android, EAS Update, App Review, and signing changes.'
}

fail() { printf 'BUILD_107_BLOCKED: %s\n' "$*" >&2; exit 1; }
pass() { printf 'PASS: %s\n' "$*"; }
...
EXPECTED_SHA="${MWM_BUILD107_SOURCE_SHA:-}"
...
[[ "$EXPECTED_SHA" =~ ^[0-9a-f]{40}$ ]] || fail "set MWM_BUILD107_SOURCE_SHA to the exact reviewed 40-character lowercase commit SHA"
...
node "$ROOT/scripts/validate-build-107-staging.cjs"
pass "Build 107 static staging/TestFlight policy"
...
node --test "$ROOT/scripts/__tests__/build-107-testflight-staging.test.cjs"
pass "frozen install, typechecks, mobile tests, audit, lint, and Build 107 policy tests"
...
node "$ROOT/scripts/validate-build-107-expo-output.cjs" \
...
node "$ROOT/scripts/validate-build-107-eas-environment.cjs" "$PROJECT_ENV_OUTPUT" "$ACCOUNT_ENV_OUTPUT"
...
pnpm exec eas build:inspect \
  ...
  --output "$ARCHIVE_DIR" \
  --force
node "$ROOT/scripts/validate-build-107-archive.cjs" "$ARCHIVE_DIR"
cd "$ROOT"
...
printf '\nIOS_BUILD_107_UPLOADED_TO_TESTFLIGHT. App Review was not started.\n'
