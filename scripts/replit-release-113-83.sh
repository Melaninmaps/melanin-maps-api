#!/usr/bin/env bash
# Mapping With Melanin — surgical release executor for iOS 1.1.9 (113)
# and Android 1.1.7 (83). This script does NOT edit application code, delete
...
require_exact_clean_source() {
  ...
  [ "$(jq -r '.expo.ios.buildNumber' artifacts/mobile/app.json)" = "113" ] || fail "iOS build number must remain 113"
  [ "$(jq -r '.expo.android.versionCode' artifacts/mobile/app.json)" = "83" ] || fail "Android versionCode must remain 83"
...
case "$MODE" in
  ...
  verify)
    ...
    node scripts/verify-release-artifacts.mjs
    printf 'VERIFY_PASS: exact_sha=%s ios=1.1.9(113) android=1.1.7(83)\n' "$RELEASE_SHA"
    ;;
  ...
  build)
    ...
    # iOS build 113: production API, exact source gate, then TestFlight submission.
    pnpm exec eas build \
      --platform ios \
      --profile production \
      --auto-submit-with-profile production \
      --what-to-test 'Build 113: Library source-backed research, Kinfolk voice and consent wording, Community immediate-feed recovery, map discovery and essential services, typo recovery, collective-opinion handling, and city briefings. Verify the fresh production API before testing.' \
      --clear-cache \
      --freeze-credentials \
      --no-wait \
      --non-interactive \
      --json > "$ROOT/release-eas-ios-113.json"

    # Android code 83: production AAB only. Do not guess a Play Console track
    ...
    pnpm exec eas build \
      ...
      --no-wait \
      --non-interactive \
      --json > "$ROOT/release-eas-android-83.json"
