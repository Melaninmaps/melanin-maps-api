...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-northeast-core-expansion-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "northeast-core-expansion-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "northeast-core-expansion-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    northeast_core_expansion_review_package: {
...
const summary = {
  package: "2026-09-18-northeast-core-expansion-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "northeast-core-expansion-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "northeast-core-expansion-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "northeast-core-expansion-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "northeast-core-expansion-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "northeast-core-expansion-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
