...
const inputManifest = resolve(consolidatedDirectory, "philly-harrisburg-support-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "philly-harrisburg-support-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    philly_harrisburg_support_review_package: {
...
// Destination health is a review gate. Persist it on the review-only manifest
// so a staging route cannot treat a timeout, network error, or non-success
// response as ready to publish.
for (const item of candidateStatuses) {
  const record = accepted[item.sourceRow - 1];
  if (record) record.sourceStatus = item.status;
}
...
const summary = {
  package: "2026-09-19-philly-harrisburg-support-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "philly-harrisburg-support-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "philly-harrisburg-support-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "philly-harrisburg-support-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "philly-harrisburg-support-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "philly-harrisburg-support-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
