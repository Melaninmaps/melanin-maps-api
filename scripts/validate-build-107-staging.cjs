...
function validateBuild107Policy() {
  ...
  for (const [name, contents] of [["root", rootEasIgnore], ["mobile", mobileEasIgnore]]) {
    for (const required of ["artifacts/web-static", "web-static", "data/founder-imports", "*.jks", "*.keystore", "*.pem", "*.p12", "*.p8", "*.key", "*.mobileprovision", "*.log", "complaint*.txt", "*_log.txt", "credentials.json", "google-service-account.json", "google-services.json", "GoogleService-Info.plist", ".replit", "replit.md", "replit.nix", "sedQ6qvzl", "upload_keystore.jks"]) {
