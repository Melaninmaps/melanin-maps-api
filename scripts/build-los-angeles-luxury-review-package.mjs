...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-19-los-angeles-luxury-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "los-angeles-luxury-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "los-angeles-luxury-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    los_angeles_luxury_review_package: {
...
const summary = {
  package: "2026-09-19-los-angeles-luxury-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "los-angeles-luxury-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "los-angeles-luxury-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "los-angeles-luxury-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "los-angeles-luxury-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "los-angeles-luxury-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
