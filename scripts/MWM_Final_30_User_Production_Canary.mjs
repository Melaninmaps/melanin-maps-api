...
function validateDeployment(version, ready) {
  ...
  if (
    !version.body?.bundle_sha256 ||
    version.body.bundle_sha256 !== version.body.bundle_sha256_self
  )
    failures.push("bundle_hash_mismatch");
  if (!ready.ok || ready.body?.status !== "ok" || ready.body?.db !== "ok")
    failures.push(`readyz_${ready.status}`);
  if ((ready.body?.pool?.max ?? ready.body?.pool_max ?? 0) < 50)
    failures.push("pool_max_below_50");
  return failures;
}
...
function phaseAbortReason(monitor) {
  ...
  if (
    recent.length === 2 &&
    recent.every((sample) => Number(sample.body?.pool?.waiting ?? 0) > 0)
  ) {
  ...
  if (
    monitor.some(
      (sample) =>
        sample.status !== 200 || sample.body?.status !== "ok" || sample.body?.db !== "ok",
    )
  ) {
...
async function simulatedUser(account) {
  ...
  try {
    ...
    for (const [name, response] of [
      ["preferences_read", prefs],
      ["sessions_read", sessions],
      ["library_graph_read", graph],
      ["business_search", business],
      ["map_discoverability_pins", pins],
      ["travel_route", travel],
    ]) {
      result.steps.push({
        name,
        ...
        retryAfter: response.headers?.retryAfter ?? null,
      });
...
const result = {
  ...
  guardrails: {
    ...
    rateLimitExpectation:
      "authenticated general API is keyed by member ID; no generic shared-IP 429s",
  },
  deployment: {},
  phases: [],
  overall: "pass",
...
result.deployment = {
  version: { status: version.status, body: version.body },
...
if (preflightFailures.length) {
  ...
  console.error(
    JSON.stringify({ overall: result.overall, abortReason: result.abortReason, output: OUTPUT }, null, 2),
  );
  process.exit(2);
}
...
for (const phase of PHASES) {
  ...
  if (priorAbort) {
    phaseResult.outcome = "abort";
    phaseResult.abortReason = priorAbort;
...
result.finishedAt = iso();
await fs.writeFile(OUTPUT, JSON.stringify(result, null, 2));
const summary = result.phases.map((p) => ({
  phase: p.name,
  outcome: p.outcome,
  plannedUsers: p.plannedUsers,
