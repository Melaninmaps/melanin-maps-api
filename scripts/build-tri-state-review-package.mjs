...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-tri-state-verified-sweep");
...
const inputHealth = resolve(consolidatedDirectory, "tri-state-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  const record = {
    ...
    instagramUrl: officialDestination(source.instagramUrl),
    facebookUrl: officialDestination(source.facebookUrl),
    tiktokUrl: officialDestination(source.tiktokUrl),
    socialSourceUrl,
...
const summary = {
  package: "2026-09-18-tri-state-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "tri-state-review-only-candidates.jsonl",
  candidates: accepted.length,
