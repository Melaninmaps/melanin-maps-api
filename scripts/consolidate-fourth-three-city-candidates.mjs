...
const root = resolve(process.argv[2] ?? "/home/ubuntu/melanin-maps-inventory-staging/data/founder-imports/2026-09-18-fourth-three-city-review");
...
for (const file of files) {
  ...
  for (const [lineIndex, line] of lines.entries()) {
    ...
    try {
      ...
      const isOnline = targetKind === "online_business";
      // Online-only services may be tied to a source area without claiming a
      // physical city or state. They remain addressless and mapless; physical
      // candidates still require a specific locality and street address.
      const required = ["sourceRow", "targetKind", "name", "country", "category", "sourceUrl"];
      if (!isOnline) required.push("city");
      const missing = required.filter((key) => !(key in record) || record[key] === null || String(record[key]).trim() === "");
      const locationProblem = isOnline ? record.address != null : !hasPhysicalStreetAddress(record.address);
  ...
  .map((item, index) => {
    ...
    const record = {
      ...
      notes: JSON.stringify({
        ...notes,
        fourth_three_city_consolidation: {
...
const summary = {
  ...
  physicalAddressCandidates: consolidated.filter((record) => record.targetKind !== "online_business").length,
  ...
  files: {
    candidates: "fourth-three-city-consolidated-candidates.jsonl",
    exactDuplicates: "fourth-three-city-exact-duplicates-removed.json",
    destinationCollisions: "fourth-three-city-destination-collisions-review.json",
    invalidRowsHeld: "fourth-three-city-invalid-or-incomplete-rows-held.json",
  },
};
