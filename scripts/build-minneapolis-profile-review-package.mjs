...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-19-minneapolis-profile-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "minneapolis-profile-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "minneapolis-profile-combined-destination-health.json");
...
const summary = {
  package: "2026-09-19-minneapolis-profile-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "minneapolis-profile-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "minneapolis-profile-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "minneapolis-profile-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "minneapolis-profile-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "minneapolis-profile-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
