...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-19-allentown-cumulative-everyday-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "allentown-cumulative-everyday-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "allentown-cumulative-everyday-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    allentown_cumulative_everyday_review_package: {
...
const summary = {
  package: "2026-09-19-allentown-cumulative-everyday-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "allentown-cumulative-everyday-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "allentown-cumulative-everyday-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "allentown-cumulative-everyday-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "allentown-cumulative-everyday-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "allentown-cumulative-everyday-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
