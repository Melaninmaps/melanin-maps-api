...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-19-philly-bucks-sixth-depth-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "philly-bucks-sixth-depth-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "philly-bucks-sixth-depth-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    philly_bucks_sixth_depth_review_package: {
...
const summary = {
  package: "2026-09-19-philly-bucks-sixth-depth-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "philly-bucks-sixth-depth-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "philly-bucks-sixth-depth-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "philly-bucks-sixth-depth-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "philly-bucks-sixth-depth-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "philly-bucks-sixth-depth-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
