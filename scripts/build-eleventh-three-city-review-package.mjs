...
const root = resolve(process.argv[2] ?? "data/founder-imports/2026-09-18-eleventh-three-city-review");
const consolidatedDirectory = resolve(root, "consolidated");
const inputManifest = resolve(consolidatedDirectory, "eleventh-three-city-consolidated-candidates.jsonl");
const inputHealth = resolve(consolidatedDirectory, "eleventh-three-city-combined-destination-health.json");
...
for (const source of sourceRecords) {
  ...
  record.notes = JSON.stringify({
    ...sourceNotes,
    review_only: true,
    eleventh_three_city_review_package: {
...
const summary = {
  package: "2026-09-18-eleventh-three-city-combined-source-backed-review",
...
const healthPayload = {
  ...
  sourceManifest: "eleventh-three-city-combined-review-only-candidates.jsonl",
  candidates: accepted.length,
...
await Promise.all([
  writeFile(resolve(outputDirectory, "eleventh-three-city-combined-review-only-candidates.jsonl"), manifestContent),
  writeFile(resolve(outputDirectory, "eleventh-three-city-combined-review-held-candidates.jsonl"), held.map((item) => JSON.stringify(item)).join("\n") + (held.length ? "\n" : "")),
  writeFile(resolve(outputDirectory, "eleventh-three-city-combined-review-summary.json"), `${JSON.stringify(summary, null, 2)}\n`),
  writeFile(resolve(outputDirectory, "eleventh-three-city-combined-destination-health.json"), `${JSON.stringify(healthPayload, null, 2)}\n`),
]);
